...
void affArtiList(Aliste *Pile)
{
    ...
    while (temp->suiv != NULL)
    {
        ...
        if (temp->date_per.aaaa != 0) printf("Date %d/%d/%d",temp->date_per.jj,temp->date_per.mm,temp->date_per.aaaa);
        temp = temp->suiv;
    }
}
...
{
    if ((int)option == 49 || (int)option == 50 ){ // force the user to give only 1 or 2 by checking ascii number of option
    Aliste *I=initAliste();
    Article x;
    short int exist=0;
    while (P->Sommet != NULL)
    ...
    if (exist==0) printf("Cet article n'appartient pas a la liste des articles\n");
     // --- PARTIE RECONSTRUCION ---
